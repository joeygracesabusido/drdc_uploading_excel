    ...
    def load_data_to_drdc_mysql():
        ...
        for index, row in data_df.iterrows():
            entry = {
                ...
                "vat_exempt": 0.0 if math.isnan(row['VAT_EXEMPT']) else row['VAT_EXEMPT'],
                "net_ofvat_with_vat_exempt": 0.0 if math.isnan(row['NET_OF_VAT_WITH_VAT_EXEMPT']) else row['NET_OF_VAT_WITH_VAT_EXEMPT'],
                "amount_due":  0.0 if math.isnan(row['AMOUNT_DUE']) else row['AMOUNT_DUE'],
                "with_holding_tax":  0.0 if math.isnan(row['WITH_HOLDING_TAX']) else row['WITH_HOLDING_TAX'],
                "total_amount_due":  0.0 if math.isnan(row['TOTAL_AMOUNT_DUE']) else row['TOTAL_AMOUNT_DUE'],
